...
- Only `enabled = Yes` accounts are used. The Accounts tab has 26 data rows: all 13 Facebook accounts (FB-ISR/JPN/KOR/RUS/PH/JIYA/MMR/GLOBAL/BKK/TREND/LTD/CD/NFCD) plus FB-INDO are enabled. Instagram IG-BKK/TREND/LTD/CD, IG-INDO, IG-RUS, IG-KOR and IG-JPN are enabled (IG-JPN's IG business account is being linked; IG-MMR/PH disabled until IG business IDs are linked). The uploader auto-resolves the IG business account from the linked page at upload time; if the page is enabled but not yet linked it raises `IGAccountNotLinkedError`, keeps the job pending (no attempt consumed) and publishes automatically once linked. YouTube YT-CD and YT-JIYA are enabled. LINE-CD, WECHAT-CD, SHOPEE-CD, LAZADA-CD and TWITCH-CD are provisioned via `add_accounts.py` but only used once their tokens/IDs are filled in (PINTEREST-CD is currently enabled).
