...
- Facebook and processed Instagram Reels are normalized to 1080x1920 without cropping the source. A blurred edge-fill background covers unused canvas space while the complete model, jewellery and text remain visible. Existing audible tracks—including quiet original audio—are preserved; music is added only when audio is missing or genuinely silent.
