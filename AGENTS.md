...
- Caption precedence (`build_caption`): primary_language translation + matching hashtags → native-language generated copy when the Source Import cell is empty → platform caption + hashtags (English accounts only) → auto-generated (no cross-language mixing). A missing regional Source cell never refuses the job and never falls back to English.
