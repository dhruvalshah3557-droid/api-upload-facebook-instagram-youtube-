...
- Only `enabled = Yes` accounts are used. The Accounts tab has 36 data rows. Enabled: all 13 Facebook accounts (FB-ISR/JPN/KOR/RUS/PH/JIYA/MMR/GLOBAL/BKK/TREND/LTD/CD/NFCD) plus FB-INDO; Instagram IG-BKK/TREND/LTD/CD, IG-INDO, IG-RUS, IG-KOR and IG-ISR; YouTube YT-CD. Disabled (provisioned but not used until tokens/IDs are filled in): YT-JIYA, IG-JPN, IG-MMR, IG-PH, IG-DUBAI, IG-KUWAIT, IG-PAK, FB-VNM, IG-VNM, LINE-CD, WECHAT-CD, SHOPEE-CD, LAZADA-CD, TWITCH-CD, PINTEREST-CD, THREADS-CD. The uploader auto-resolves the IG business account from the linked page at upload time; if the page is enabled but not yet linked it raises `IGAccountNotLinkedError`, keeps the job pending (no attempt consumed) and publishes automatically once linked.
