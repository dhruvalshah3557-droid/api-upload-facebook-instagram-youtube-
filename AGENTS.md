...
- Instagram catalog music: silent Reels first try Meta's authorized `/ig_audio` catalog. `IG_AUDIO_SEARCH_QUERIES` may contain comma/newline-separated mood searches; a SHA-256 hash of the media URL selects both query and a track from all returned results. This prevents the former `tracks[0]` behavior that repeated one song. If the catalog is unavailable, the Reel uses the bundled rotating CC0 library via `prepare_video()`.
- Minimum market delivery: `optimized_runner.py` counts confirmed `uploaded` queue rows in a rolling 24-hour UTC window and prioritizes `IG-SPAIN`, `IG-ITALY`, `IG-VNM`, `IG-PAK`, `IG-KUWAIT`, and `IG-DUBAI` until each reaches at least 3 successful posts. Deficit posts use a 5-hour minimum spacing so the three posts are distributed instead of dumped together. Duplicate-media locks and normal media validation still apply.
...
- LINE (`line_uploader.py`): LINE-CD is enabled. Broadcasts via Messaging API to all followers. Caption text is sent first, then media. No native carousel — a carousel is sent as caption + up to 5 image messages per broadcast request. Media URLs must be publicly reachable HTTPS URLs. Videos need a JPEG/PNG preview (product main image). Production `optimized_runner.py` reserves 1 LINE slot per 20-job run. Secrets: `LINE_CHANNEL_ACCESS_TOKEN` and/or `META_TOKEN_LINE_CD`. Use `python enable_line_account.py` (workflow Enable LINE Account) to set Accounts `LINE-CD.enabled = Yes` without rewriting other rows.
